...
#### 13:28
I hardly know what to try anymore, I'm just testing random things and hoping for the best. Little setback, though, is that GitHub Pages doesn't update accordingly sometimes, so even if i Push an updated version of a file and can see that the file has updated successfully on GitHub, the Pages site is sometimes yet to update. This is despite the green check mark being present, which I think is meant to tell that GitHub Pages has finished loading. I'm not sure if it's because of me committing and pushing every other minute (I think i've committed and pushed a file about 15 times in the past 30 minutes by now), or something else. Regardless, I'm still debugging this service worker. Trying to, at least.

#### 13:35
And it works, I think? If I didn't know any better, I'd swear that service workers are magic. Anyhow, I won't question it. If it works, it's good enough for me. Now that all elements appear to be in place (I think), I'll try downloading the PWA on my phone and hopefully be done.
* https://stackoverflow.com/questions/50552532/service-worker-not-registering-on-gh-pages-hosted-website
* https://stackoverflow.com/questions/61276173/failed-to-register-a-serviceworker-for-scope-https-xxx-net-firebase-cloud-m
* https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API/Using_Service_Workers